...
window.importMembers = () => {
    ...
    reader.onload = async function(e) {
        ...
        // Basic Validation: Check if there's data
        ...
        // Expected Format: Name, Phone, Gender, JoinDate(YYYY-MM-DD), Plan(1m/3m), Amount
        for (let i = 1; i < rows.length; i++) {
            const cols = rows[i].split(',');
            if(cols.length < 5) continue; // Skip malformed rows

            // Clean data (remove quotes, trim)
            ...
            if(name && phone && joinDate) {
                try {
                    ...
                    const val = parseInt(plan); 
                    // Simple parsing logic matching formatPlanDisplay logic
                    if(plan.includes('d')) d.setDate(d.getDate() + val);
                    else if(plan.includes('y')) d.setFullYear(d.getFullYear() + val);
                    else d.setMonth(d.getMonth() + (val || 1)); // Default 1 month if parsing fails
                    ...
                    const docRef = await addDoc(collection(db, `gyms/${currentUser.uid}/members`), {
                        ...
                        memberId: memberId,
                        createdAt: new Date(),
                        photo: null // No photo import support yet
                    });

                    // 2. Add Finance Entry automatically for this import
                    if(amount > 0) {
...
window.generateInvoice = async (m, specificTransaction = null) => {
    ...
    try {
        ...
        // Position Image immediately BELOW text
        doc.addImage(signImg, 'JPEG', 150, finalY + 2, 40, 15); 
    } catch(e) { console.log("Sign error", e); }
    doc.line(150, finalY + 18, 196, finalY + 18); 
